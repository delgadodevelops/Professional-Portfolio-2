      ...
      <div className="p-5 m-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-16">
          {skills.map((skill, index) => (
            <div
              key={index}
              className="bg-custom-blue p-6 rounded-lg shadow-md text-center text-white hover:bg-custom-orange hover:scale-110 hover:duration-700 flex flex-col justify-center items-center sm:w-52 sm:h-52 sm:text-lg"
            >
              <img
                src={skill.imageUrl}
                alt={`Image for ${skill.name}`}
                className="h-10 w-10 mb-2 mx-auto sm:h-20 sm:w-20 sm:mb-7 lg:h-22 lg:w-24 lg:mb-8 "
              />
